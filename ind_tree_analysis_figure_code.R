...
tiff(filename="./plots/Figure_5_pdc_mort_effects.tif", 
     type = "cairo",
     antialias = "gray",
     compression = "lzw",
     units="in", 
...
vpd_peak_anom_mean <- mean(all_tree_vars$vpd_peak_anom)
vpd_peak_anom_sd <- sd(all_tree_vars$vpd_peak_anom)
...
ENN_dist_mean <- mean((all_tree_vars_trans[all_tree_vars_trans$Spp == "JUOS", "ENN_dist"]))
ENN_dist_sd <- sd((all_tree_vars_trans[all_tree_vars_trans$Spp == "JUOS", "ENN_dist"]))
...
#   C$"(Intercept)" <- 1
#   C$Avg_depth <- unname(quantile(alldata$Avg_depth, quant))
#   C$avg_BA_4m <- minmax.sequence(alldata, "avg_BA_4m", nrow(C))
#   C$"Avg_depth:avg_BA_4m" = C$Avg_depth * C$avg_BA_4m
#   
...
#     axis(2)
#     mtext(side = 2, text = "Predicted Change in Mean Canopy", cex = 0.7, outer = FALSE, line = 1.7)}
#   
#   #label x axis
...
calculate_preds_BA_4m <- function(quant){
  ...
  C$BA_4m_log <- seq(-3.5, max(subset(all_tree_vars, select = "BA_4m_log")), length.out = 1000)
  ...
  preds <- data.frame(
    ...
    BA_4m_sim_natural_scale = (exp((pred$BA_4m_sim * BA_4m_sd) + BA_4m_mean)/10000) # in m2
  )
...
for(i in 1:3){
  ...
  text(x = 0.05, y = -3, labels = paste0(values[i]*100, "% CWD = \n", 
...
## figure for presentations -- remove for paper submission
...
for(i in 1:3){
  ...
  lines((pred$hi) ~ pred$BA_4m_sim_natural_scale, lwd = 1.3)
  polygon(c(pred$BA_4m_sim_natural_scale, rev(pred$BA_4m_sim_natural_scale)), c((pred$hi), rev((pred$lo))),
          col = addTrans("blue",30), border = NA) #fills in the area between high and low confidence intervals
  
  
  text(x = .05, y = -3, labels = paste0(values[i]*100, "% CWD = \n", 
                                        round(((unname(quantile(all_tree_vars$cwd_normal_cum, values[i])) * cwd_normal_cum_sd + cwd_normal_cum_mean)), 0), " mm"),
       cex = 1)
  
  ticks <- seq(-3, 0, by=1)
  labels <- sapply(ticks, function(i) as.expression(bquote(10^ .(i))))
  axis(1, at=c(0.001, 0.01, .1, 1), labels=labels)
  
  if(i == 1){
    axis(2)
    mtext(side = 2, text = "Predicted Change in Canopy", cex = 1.3, outer = FALSE, line = 2)}
  
  #label x axis
  if(i == 2){mtext(side = 1, text = expression(paste("BA, 4m buffer (", m^2, ")")), cex= 1.3, outer = FALSE, line = 2.4)}
...
# }
  ...
  points(part_resids ~ cwd_normal_cum_orig, pch = 20, cex = 2, col = "grey70")
  #draw prediction line and confidence envelope of predictions
  ...
  axis(1, cex.axis = 1.4) #draw the x-axis
  axis(2, cex.axis = 1.4)
