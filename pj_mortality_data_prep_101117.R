...
write.csv(alldata, "./Clean data/plot_level_vars.csv")

plot_data_scaled <- alldata

#scale all the plot-level data, except the response variables
plot_data_scaled[, -c(23,24)][, sapply(plot_data_scaled[, -c(23,24)], is.numeric)] <- #scale everything
  scale(alldata[, -c(23,24)][, sapply(alldata[, -c(23,24)], is.numeric)])
...
write.csv(all_tree_vars_orig, "./Clean data/all_tree_vars_unscaled_untrans.csv")
