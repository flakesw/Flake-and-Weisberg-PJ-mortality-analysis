...
plot(resid(plot_model)~predict(plot_model_dredge)) #looks good!
abline(h=0)
...
plot_model_int_depth <- update(plot_model_dredge, . ~ . + avg_BA_4m:Avg_depth)

saveRDS(plot_model_int_depth, "./model output/plot_model.rds")

plot_model_int_cwd <- update(plot_model_dredge, . ~ . + avg_BA_4m:cwd_normal_cum)
plot_model_int_tmax <- update(plot_model_dredge, . ~ . + avg_BA_4m:avg_summer_tmax)
...
r.squaredGLMM(plot_model_int_depth)
summary(plot_model_int_depth)
...
bootstrap_plot_level <- boot(data= plot_data_scaled, 
                             statistic=bs, 
                             R=5000, 
                             formula=plot_model_int_depth@call,
                             parallel = "no")
