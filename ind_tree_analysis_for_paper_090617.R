...
pimo_dredge <- dredge(pimo_global, 
                      # fixed = c('Neighbor_larger_log', 'ENN_dist_log','BA_4m_log'),
                      subset = cormat, 
                      m.lim = c(5, 9), 
                      trace = 2)
...
pimo_model <- eval(parse(text = best_pimo_call))
summary(pimo_model)
...
juos_model <- eval(parse(text = best_juos_call))

j1 <- lmer(Delta_pdc ~ fdsi_anom + avg_summer_tmax + Avg_depth + AWC + 
             (1|site/Cluster), data = all_tree_vars[all_tree_vars$Spp == "JUOS", ], na.action = na.fail)
# j2 <- lmer(Delta_pdc ~ avg_summer_tmax + AWC + fdsi_anom + vpd_peak_anom +
#              (1|site/Cluster), data = all_tree_vars[all_tree_vars$Spp == "JUOS", ], na.action = na.fail)
# j3 <- lmer(Delta_pdc ~ Avg_depth + avg_summer_tmax + fdsi_anom + vpd_peak_anom +
#              (1|site/Cluster), data = all_tree_vars[all_tree_vars$Spp == "JUOS", ], na.action = na.fail)
# j4 <- lmer(Delta_pdc ~ avg_summer_tmax + AWC + fdsi_anom + 
#              (1|site/Cluster), data = all_tree_vars[all_tree_vars$Spp == "JUOS", ], na.action = na.fail)
# j5 <- lmer(Delta_pdc ~ Avg_depth + avg_summer_tmax + fdsi_anom + 
#              (1|site/Cluster), data = all_tree_vars[all_tree_vars$Spp == "JUOS", ], na.action = na.fail)
# j6 <- lmer(Delta_pdc ~ avg_summer_tmax + fdsi_anom + vpd_peak_anom +
#              (1|site/Cluster), data = all_tree_vars[all_tree_vars$Spp == "JUOS", ], na.action = na.fail)
# j7 <- lmer(Delta_pdc ~ avg_summer_tmax + fdsi_anom + 
#              (1|site/Cluster), data = all_tree_vars[all_tree_vars$Spp == "JUOS", ], na.action = na.fail)
# j8 <- lmer(Delta_pdc ~ Avg_depth + fdsi_anom + ppt + vpd_peak_anom +
#              (1|site/Cluster), data = all_tree_vars[all_tree_vars$Spp == "JUOS", ], na.action = na.fail)
# j9 <- lmer(Delta_pdc ~ Avg_depth + fdsi_anom + min_ann_ppt + vpd_peak_anom +
#              (1|site/Cluster), data = all_tree_vars[all_tree_vars$Spp == "JUOS", ], na.action = na.fail)
# j10 <- lmer(Delta_pdc ~ Avg_depth + AWC + fdsi_anom + ppt + 
#              (1|site/Cluster), data = all_tree_vars[all_tree_vars$Spp == "JUOS", ], na.action = na.fail)
# 
# max(vif.mer(j1))
# 
...
pimo_mort_clim_global <- glmer(Died ~ cwd_normal_cum + fdsi_anom +
                              cwd_peak_anom + tmean + ppt +
                              vpd_normal_annual_max + vpd_peak_anom + avg_summer_tmax +
                              peak_tmax + min_ann_ppt +
                              Pndjfm + Avg_depth + AWC + (1|site), family = "binomial",
                              data = all_tree_vars[all_tree_vars$Spp == "PIMO", ], na.action = na.fail)

summary(pimo_mort_clim_global)

cormat <- abs(cor(all_tree_vars[all_tree_vars$Spp == "PIMO", ][, c('cwd_normal_cum', 'fdsi_anom',
                                                                   'cwd_peak_anom' , 'tmean', 'ppt',
                                                                   'vpd_normal_annual_max' , 'vpd_peak_anom' , 'avg_summer_tmax', 'peak_tmax',
                                                                   'min_ann_ppt',  'Pndjfm' , 'Avg_depth' , 'AWC')])) <=.5
cormat[!lower.tri(cormat)] <- NA
...
start.time <- Sys.time()
p_mort_dredge <- dredge(pimo_mort_clim_global, subset = cormat, m.lim = c(0, 3))
(p_mort_dredge)[c(1:10)]

end.time <- Sys.time()
time.taken_pmort_dredge <- end.time - start.time
time.taken_pmort_dredge #2.4 minutes


# Old results with 4 variables
pm1 <- glmer(Died ~ Avg_depth + AWC + cwd_normal_cum + fdsi_anom +
               (1|site), family = "binomial",
             data = all_tree_vars[all_tree_vars$Spp == "PIMO", ], na.action = na.fail)

pmnew <- glmer(Died ~ Avg_depth + cwd_normal_cum + fdsi_anom +
                 (1|site), family = "binomial",
               data = all_tree_vars[all_tree_vars$Spp == "PIMO", ], na.action = na.fail)

# pm2 <- glmer(Died ~ Avg_depth + cwd_normal_cum + fdsi_anom + 
#                (1|site), family = "binomial", 
#              data = all_tree_vars[all_tree_vars$Spp == "PIMO", ], na.action = na.fail)
# pm3 <- glmer(Died ~ Avg_depth + cwd_normal_cum + fdsi_anom + peak_tmax + 
#                (1|site), family = "binomial", 
#              data = all_tree_vars[all_tree_vars$Spp == "PIMO", ], na.action = na.fail)
# pm4 <- glmer(Died ~ Avg_depth + fdsi_anom + cwd_peak_anom + peak_tmax + 
#                (1|site), family = "binomial", 
#              data = all_tree_vars[all_tree_vars$Spp == "PIMO", ], na.action = na.fail)
# pm5 <- glmer(Died ~ Avg_depth + fdsi_anom + AWC + peak_tmax + 
#                (1|site), family = "binomial", 
#              data = all_tree_vars[all_tree_vars$Spp == "PIMO", ], na.action = na.fail)
# pm6 <- glmer(Died ~ Avg_depth + fdsi_anom + AWC +  
#                (1|site), family = "binomial",  
#              data = all_tree_vars[all_tree_vars$Spp == "PIMO", ], na.action = na.fail)
# pm7 <- glmer(Died ~ Avg_depth + fdsi_anom + AWC + cwd_peak_anom +
#                (1|site), family = "binomial", 
#              data = all_tree_vars[all_tree_vars$Spp == "PIMO", ], na.action = na.fail)
# pm8 <- glmer(Died ~ Avg_depth + fdsi_anom + peak_tmax +
#                (1|site), family = "binomial", 
#              data = all_tree_vars[all_tree_vars$Spp == "PIMO", ], na.action = na.fail)
# pm9 <- glmer(Died ~ Avg_depth + fdsi_anom + 
#                (1|site), family = "binomial", 
#              data = all_tree_vars[all_tree_vars$Spp == "PIMO", ], na.action = na.fail)
# pm10 <- glmer(Died ~ Avg_depth + fdsi_anom + cwd_peak_anom +
#                 (1|site), family = "binomial", 
#              data = all_tree_vars[all_tree_vars$Spp == "PIMO", ], na.action = na.fail)
# max(vif.mer(pm10))
# 
...
#Tree-level models
#############################################################################################
### These models came from above climate models, taking model 1 (best AICc) and adding the
# stand- and tree-level variables to that model and refitting

#PIMO dieback

# pimo_model <- lmer(formula = Delta_pdc ~ Neighbor_larger + ENN_dist + Height + Diam + BA_4m +  
#                      Avg_depth + cwd_normal_cum + vpd_normal_annual_max + vpd_peak_anom + (1|Cluster/site), data = all_tree_vars[all_tree_vars$Spp == "PIMO", ])
# summary(pimo_model, ddf = "Kenward-Roger")
# AICc(pimo_model)


pimo_int_ba <- lmer(formula = Delta_pdc ~ Neighbor_larger + ENN_dist + Height + Diam + BA_4m +  
       Avg_depth + cwd_normal_cum + vpd_normal_annual_max + vpd_peak_anom + 
       BA_4m*cwd_normal_cum + (1|Cluster/site), data = all_tree_vars[all_tree_vars$Spp == "PIMO", ])
...
pimo_int_ba_new <- lmer(formula = Delta_pdc ~ Neighbor_larger + ENN_dist + Height + Diam + BA_4m +  
                      Avg_depth + cwd_normal_cum + vpd_normal_annual_max + 
                      BA_4m*cwd_normal_cum + (1|Cluster/site), data = all_tree_vars[all_tree_vars$Spp == "PIMO", ])

# ## significant interaction (improves AIC)
# summary(pimo_int_ba)
# vif.mer(pimo_int_ba)
# 
# 
# pimo_int_enn <- lmer(formula = Delta_pdc ~ Neighbor_larger + ENN_dist + Height + Diam + BA_4m +  
#        Avg_depth + cwd_normal_cum + vpd_normal_annual_max + vpd_peak_anom + 
#        ENN_dist*Neighbor_larger + (1|Cluster/site), data = all_tree_vars[all_tree_vars$Spp == "PIMO", ])
# # no interaction with neighbor larger
# summary(pimo_int_ba)
...
bs(formula = Delta_pdc ~ Neighbor_larger + ENN_dist + Height + Diam + BA_4m +  
     Avg_depth + cwd_normal_cum + vpd_normal_annual_max + vpd_peak_anom + cwd_normal_cum*BA_4m + (1|Cluster/site),
   data = all_tree_vars[all_tree_vars$Spp == "PIMO", ])
...
bootstrap_pimo_int <- boot(data=all_tree_vars[all_tree_vars$Spp == "PIMO", ], 
                                statistic=bs, 
                                R=5000, 
                                formula=Delta_pdc ~ Neighbor_larger + ENN_dist + Height + Diam + BA_4m +  
                                  Avg_depth + cwd_normal_cum + vpd_normal_annual_max + vpd_peak_anom + cwd_normal_cum*BA_4m + (1|Cluster/site),
                                parallel = "no")
end.time <- Sys.time()
...
write.csv(ci_pmort, "./model output/ci_pimo.csv")
...
#--------------------------------------------------------------
# JUOS dieback
# juos_model <- lmer(formula = Delta_pdc ~ Neighbor_larger + ENN_dist + Height + Diam + BA_4m +  
#                      fdsi_anom + avg_summer_tmax + Avg_depth + AWC + (1|Cluster/site), 
#                    data = all_tree_vars[all_tree_vars$Spp == "JUOS", ])
# # summary(juos_model)
# # AICc(juos_model)
# # vif.mer(juos_model)
# 
# juos_int_ba <- lmer(formula = Delta_pdc ~ Neighbor_larger + ENN_dist + Height + Diam + BA_4m +  
#                      fdsi_anom + avg_summer_tmax + Avg_depth + AWC + 
#                       BA_4m*avg_summer_tmax +(1|Cluster/site), 
#                    data = all_tree_vars[all_tree_vars$Spp == "JUOS", ])
# ## no significant interactions with BA_4m for juniper
# 
# # AICc(juos_int_ba)
juos_int_enn <- lmer(formula = Delta_pdc ~ Neighbor_larger + ENN_dist + Height + Diam + BA_4m +
                      fdsi_anom + avg_summer_tmax + Avg_depth + AWC +
                      ENN_dist*Neighbor_larger +(1|Cluster/site),
                    data = all_tree_vars[all_tree_vars$Spp == "JUOS", ])
...
#full model
p_mort <- glmer(Died ~ Neighbor_larger + ENN_dist + Height + Diam + BA_4m +
                    Avg_depth + AWC + cwd_normal_cum + fdsi_anom + (1|site), family = "binomial",
                data = all_tree_vars[all_tree_vars$Spp == "PIMO", ])
  summary(p_mort, ddf = "Kenward-Roger")
  
  
pmort_new <- glmer(Died ~ Neighbor_larger + ENN_dist + Height + Diam + BA_4m +
                     Avg_depth + cwd_normal_cum + fdsi_anom + (1|site), family = "binomial",
                   data = all_tree_vars[all_tree_vars$Spp == "PIMO", ])
summary(pmort_new, ddf = "Kenward-Roger")
